...
## 界面术语
   ...
   * 如果这个接口返回的是数据就需要开启restful 选项，然后勾选对应的权限，不勾选对应的权限是不能访问的

## 初出茅庐

   这里我们讲介绍系统的4大基础字段属性，掌握了这些基础字段，就可以尝试建立一些常规的接口，给客户端使用
...
* 第三式 `integer` 适用于各种id，但是有一点要注意数据的长度不要超过11位 或者数值在 -2^31 (-2,147,483,648) 到 2^31 – 1 (2,147,483,647)之间，不然数据会不正常的，然后你整个人就不好了
...
例如 img string 200|xxx.png ,这里我们创建了一个长度为200的img字段，若img创建的时候没有手动赋值将会默认使用xxx.png作为值，这个会在后面有妙用，请少侠牢记
