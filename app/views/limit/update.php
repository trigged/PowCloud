...
<div class="">
    ...
    <form data-status="0" id="group_form" class="form-horizontal"  onsubmit="return check_form(this)">
        <div class="control-group">
            ...
            <div class="controls">
                <input name="group[groupName]" class="input-medium" value="<?php echo $limit->groupName; ?>" type="text" placeholder="用户组名称" id="name">
            </div>
        </div>
        <?php foreach ($models as $model): ?>
            <div class="control-group">

                <label for="<?php echo $model->table_name; ?>" class="control-label"><?php echo $model->table_alias; ?>*:</label>
                <div class="controls">
                    <?php
                    ...
                    ?>
                    <?php echo Form::radio('limit['.$model->table_name.']',1, ($default_check)?'checked':''); ?>无权限
                    <?php echo Form::radio('limit['.$model->table_name.']',2, (isset($dataModel[$model->table_name]['read']) && $dataModel[$model->table_name]['read']==2 ?'checked':'')); ?>只看权限
                    <?php echo Form::radio('limit['.$model->table_name.']',3, (isset($dataModel[$model->table_name]['edit']) && $dataModel[$model->table_name]['edit']==2 ?'checked':'')); ?>修改权限
                </div>
            </div>
        <?php endforeach; ?>
