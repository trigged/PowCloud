...
<form class="form-horizontal child_form" action="<?php echo URL::action('FormsController@store')?>"  method="post" onsubmit="return check_form(this);">
    <fieldset>
    ...
    <fieldset>
        <legend>表单字段</legend>
            <table class="table table-striped">
                ...
                <tbody>
                <?php $index = 0;foreach($formFields as $formField ):?>
                <tr>
                    <td class="">
                        <?php echo $formField['name']?>(<?php echo $formField['type']->getName(); ?>)
                        <input type="hidden"  name="field[<?php echo $index;?>][field]" value="<?php echo $formField['name']?>" />
                        <input type="hidden"  name="field[<?php echo $index;?>][models_id]" value="<?php echo $table->id;?>" />
                    </td>
                    <td>
                        <input data-tip="标签(<?php echo $formField['name']; ?>)" type="text" class="input-mini required" name="field[<?php echo $index;?>][label]" value=""  />
                    </td>
                    <td>
                        <?php echo Form::select('field['.$index.'][dataType]',Config::get('params.dataType'),'',array('class'=>'input-small')); ?>
                    </td>
                    <td>
